...
Welcome to SPACE: Space Program for Aeronautical Collection and Exploration. You are in charge of the Rover Control Center. All rovers on the Moon and Mars are under your direction!
...
Working with Rovers
1.
There are two Moon rovers and one Mars rover defined in Program.cs. To manage them all at once, we’ll need to put them in an array.

Store the three rovers in an array named rovers of type Rover[], their shared base class. For now, ignore the Satellite object.

2.
You’ll regularly need to direct the rovers to send info, explore their surroundings, and collect samples.

Below Main(), write a static method DirectAll() that takes one argument of type Rover[]. The method should call the following three methods for each element and print out the returned strings.
...
To manage all space probes, which includes the three rovers and one satellite, put them in an array named probes.
...
Back in Program.cs, create an array of type IDirectable[] named directables that contains all the probes (3 rovers and 1 satellite).
...
namespace RoverControlCenter
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // 1. 
            // Store the three rovers in an array named rovers of type Rover[], their shared base class.
            // For now, ignore the Satellite object.
            ...
            // Explore()
            // Collect()
            static void DirectAll(IDirectable[] arrayParameter)
            {
                foreach (IDirectable item in arrayParameter)
                {
            ...
            // Try to answer this question on your own: in task 3, you called the Explore() and Collect() methods on each rover. 
            // Even though you used Rover references, the specific rover type (MoonRover or MarsRover) was printed. 
            // How is that possible?

            // The reference type controls access.
            // The object type controls behavior.
